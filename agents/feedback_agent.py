...
def generate_feedback(all_scores, domain="industry"):
    ...
    avg_speech = sum(float(s.get("speech_clarity", 0)) for s in all_scores) / total_questions
    ...
    weak_speech = sum(1 for s in all_scores if _to_percent(s.get("speech_clarity", 0)) < 50)
    ...
    merged_strengths = _merge_unique(llm_feedback.get("strengths", []), deterministic_strengths, limit=5)
    merged_weaknesses = _merge_unique(llm_feedback.get("weaknesses", []), deterministic_weaknesses, limit=5)
    merged_plan = _merge_unique(llm_feedback.get("improvement_plan", []), deterministic_plan, limit=5)

    if summary_data["avg_speech_clarity_pct"] >= 75 and summary_data["weak_speech_clarity_count"] <= 1:
      merged_weaknesses = [w for w in merged_weaknesses if "speech" not in w.lower() and "spoken" not in w.lower()]

    if summary_data["avg_confidence_pct"] >= 75 and summary_data["weak_confidence_count"] <= 1:
